...
FROM 
  ...
  const pspdtnquery = `
  SELECT q2.item_name,q2.um,q1.qtyrecd1 as prod_qty,round(q2.qtyrecd1,2) as Month_qty,month_Avg FROM (select 
...
//Title= power plant Production
     ...
     SELECT q2.item_name,q2.um,q1.qtyrecd1 as prod_qty,round(q2.qtyrecd1,2) as Month_qty,month_Avg 
         FROM (select 
        case when item_name='DENATURED ANHYDROUS ETHANOL (PRODUCED FROM MAIZE)(4PPM BRUCINE SULPHATE)' then 'ETHANOL' ELSE upper(ITEM_NAME) END AS ITEM_NAME,
        case when UM='KGS' THEN 'MT' ELSE UM END AS UM,
        case when UM='KGS' THEN QTYRECD/1000 ELSE QTYRECD END AS QTYRECD1
...
//Title= power plant consumption
       ...
       round(q2.qtyissued1,2) as Month_qty,month_Avg 
        FROM (
         ...
         <table style="width: 100%; border-collapse: collapse; font-size: 12px; color: #333;">
             <thead>
                 <tr style="background-color: #4f82bb; color: #fff;">
                     ...
                     <th style="border: 1px solid #fff; padding: 8px; text-align: center;">Monthly Quantity</th>
                     <th style="border: 1px solid #fff; padding: 8px; text-align: center;">Monthly Avg</th>
                 </tr>
             </thead>
             <tbody>
                ${rows2}
...
async function missendEmail(htmlTable) {
  ...
  try {
    ...
    console.log(formattedEmailDate);
    const mailOptions = {
...
async function sendEmail_monthly(htmlContent) {
  try {
    const data = await fetchData();
    const htmlTable = formatDataAsHTMLTable(data);
    missendEmail(htmlTable);
  } catch (error) {
